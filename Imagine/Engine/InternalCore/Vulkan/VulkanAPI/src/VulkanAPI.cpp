...
namespace Vulkan {
	...
	void GraphicsAPI::Draw(const Mesh& mesh) {
	...
	void GraphicsAPI::InitSyncObjects() {
	...
	void GraphicsAPI::UpdateSwapchain() const {
	...
	void GraphicsAPI::CompileShaders() {
		//Needs to be awaited
		std::system("ShaderCompilation.bat");
	}

	void GraphicsAPI::OnWindowResize(GLFWwindow* window, int width, int height) {
		reinterpret_cast<GraphicsAPI*>(glfwGetWindowUserPointer(window))->m_WindowWasResized = true;
	}
